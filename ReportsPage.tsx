...
const GrowthInsightPage: React.FC = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                setError('Gagal memuat data untuk insight.');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const financialData = useMemo(() => {
        ...
        const filteredSalesOrders = salesOrders.filter(order => {
            const orderDate = new Date(order.tglPesan);
        ...
        const totalSalesOrderValue = filteredSalesOrders.reduce((sum, order) => sum + order.total, 0);
        ...
        const unspentMarketingBudget = totalSettlements - totalSalesOrderValue;
        const profitOrLoss = cashFlowMetrics.totalIncome - cashFlowMetrics.totalExpense;

        return { ...cashFlowMetrics, profitOrLoss, unspentMarketingBudget, totalSalesOrderValue };
    }, [cashFlowEntries, salesOrders, activeDateRange, accessibleWallets]);

    // FIX: Made the sort comparison functions more robust by ensuring values are treated as numbers, preventing potential arithmetic errors on non-numeric types.
        ...
        <Card>
            ...
            <CardContent className="space-y-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <MetricCard title="Unspent Marketing Budget" value={financialData.unspentMarketingBudget} color={financialData.unspentMarketingBudget >= 0 ? 'text-foreground' : 'text-destructive'}/>
                </div>
