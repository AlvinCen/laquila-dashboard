            ...
            <div className="overflow-x-auto border rounded-lg">
                <table className="w-full text-sm text-left min-w-[900px]">
                    <thead className="text-xs text-gray-900 font-medium uppercase bg-gray-50">
                        <tr>
                            {/* FIX: Changed sortKey from noInvoice to invoiceNumber. */}
                            <SortableHeader sortKey="invoiceNumber">No Invoice</SortableHeader>
                            <SortableHeader sortKey="createdAt">Tgl Pesan</SortableHeader>
                        ...
                        {isLoading ? (
                            ...
                            orders.map((order) => (
                                <tr key={order.id} className="border-b bg-white hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-2 font-medium">{order.invoiceNumber}</td>
                                    <td className="px-4 py-2">{new Date(order.createdAt).toLocaleString('id-ID', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' })}</td>
