    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">Invoice: {order.noInvoice}</p>
            </div>
            ...
            <div className="grid gap-1.5">
                ...
                <Input value={`Pelunasan untuk Invoice ${order.noInvoice}`} readOnly disabled />
            </div>
...
const SettlementPage: React.FC = () => {
        ...
        const filteredList = searchQuery
            ? settlementCandidates.filter(order => {
                ...
                return (
                    ...
                    order.noInvoice.toLowerCase().includes(lowerQuery) ||
                    order.noIdPesanan.toLowerCase().includes(lowerQuery)
                );
            })
            : settlementCandidates;
        
        return filteredList.sort((a, b) => new Date(b.tglPesan).getTime() - new Date(a.tglPesan).getTime());
    ...
    return (
        <>
                                ...
                                <div key={order.id} className="border rounded-lg p-4 flex flex-col sm:flex-row justify-between sm:items-center gap-4 hover:bg-gray-50">
                                    <div className="grid grid-cols-1 gap-y-4 text-sm sm:flex-grow sm:grid sm:grid-cols-4 sm:gap-x-4 sm:gap-y-2">
                                        <div>
                                            ...
                                            <p className="text-muted-foreground">{order.noInvoice} / {order.noIdPesanan}</p>
                                        </div>
                                        <div>
                                            <p className="text-gray-800">{new Date(order.tglPesan).toLocaleDateString('id-ID', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
                                    ...
                                    <div className="flex-shrink-0 flex sm:flex-col items-center justify-between sm:text-right gap-2 pt-4 sm:pt-0 border-t sm:border-t-0">
                                        ...
                                        {canSettle && (
                                            <Button 
                                                ...
                                                size="sm"
                                                data-testid={`settle-button-${order.noInvoice}`}
                                            >
                                                Lunasi
                                            </Button>
                                        )}
            ...
            {selectedOrder && (
                <Modal title={`Pelunasan untuk ${selectedOrder.noInvoice}`} onClose={() => setSelectedOrder(null)}>
                    <SettlementForm 
                        order={selectedOrder}
                        wallets={wallets}
