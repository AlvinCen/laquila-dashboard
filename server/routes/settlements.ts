...
export default async function settlementRoutes(server: FastifyInstance) {
  server.post('/', async (request, reply) => {
    ...
    const runTransaction = db.transaction(() => {
      ...
      const category = db.prepare('SELECT name FROM finance_categories WHERE rowid = ?').get(categoryId);
      if (!category) {
      ...
      db.prepare('UPDATE orders SET jumlahDilunasi = ?, paymentStatus = ?, orderStatus = ? WHERE id = ?')
        .run(newJumlahDilunasi, newPaymentStatus, updatedOrderStatus, salesOrderId);
